...
def create_task(project_id: int, title: str, description: str) -> Dict[str, Any]:
    """Create a new task."""
    sql = text("""
        INSERT INTO tasks (name, description, project_id, created_at, status)
        VALUES (:name, :description, :project_id, :created_at, 'pending')
        RETURNING id, name, description, status, due_date, created_at, project_id, job_id
    """)
    result = db.session.execute(sql, {
        "name": title,
        "description": description,
